...
- [ ] **Make the tenure-success floor age-aware.** Today the floor in `_apply_tenure_success_floor` (`api/ml/scoring.py`) blanket-applies whenever `tenure_days >= 1460 AND minutes_pct >= 55%` — independent of player age. This creates a false-positive risk: a young player who plays 4 years at 60% minutes and is sold below cost should NOT have their financial outcome floored away (it likely signals regression / attitude issues / failure to develop). An older player (28+) sold below cost after the same stint IS age-appropriate depreciation and the floor is correct.

  **Reference false-positive: Paul Pogba → Manchester United 2016.** Bought €105M (world record at the time), sold for €0 in 2022 at age 29. Currently grades **B 77.91** because both `value_trajectory` and `financial_return` get floored to 78 (he was 23.4 at transfer, tenure 5.9y, minutes_pct 57.6%). Without the floor: composite drops to ~C 50 — which is a defensible grade for "decent contributions, financial disaster at peak-development age."

  **Hard part of the fix: Pogba (23.4) and Rodri (23.4) have nearly identical ages but opposite truths.** A simple age cutoff won't work. The signal that distinguishes them is *severity* of the financial outcome relative to expectation: Pogba's `exit_ratio_vs_expected` is catastrophic (-0.5+), Rodri's is null (in-progress). Possible shape: scale the floor's effect by `(1 - severity)` where severity is how bad the financial outcome is. Or only floor `financial_return` when the raw score is in the [20, 50] band (moderate underperformance, not catastrophic).

  **Validation sketch:**
  1. Re-run curated list — none should regress (Rodri / De Bruyne / Alisson are all in-progress or older).
  2. Confirm Pogba lands in C/D range.
  3. Spot-check Mbappé (A 86.7) and Ederson (B+ 78.5) — should stay high. Their floor lift is small relative to the production+minutes contributions.
  4. Spot-check Xhaka and Dybala (borderline cases) — defensible if they shift down to B/C+.
...
- [ ] **Switch bulk ingest to `COPY` + temp-table `INSERT … SELECT … ON CONFLICT`.** Current path (`upsert_chunk` in `api/pipeline/upsert.py`) uses chunked `INSERT … VALUES … ON CONFLICT` — Python parses each row, builds a dict, dispatches a chunk, parses `RETURNING xmax`. Per-row overhead is ~130μs at appearances scale (1.86M rows → 246s). Postgres `COPY FROM STDIN` is ~5–10× faster on the wire because it bypasses parameter parsing and uses a binary-ish protocol. The pattern: (a) `CREATE TEMP TABLE _stage_<table> (LIKE <table>) ON COMMIT DROP`, (b) `psycopg2.copy_expert("COPY _stage_<table> FROM STDIN", buffer)` with all rows from a chunk (or the whole CSV), (c) `INSERT INTO <table> SELECT * FROM _stage_<table> ON CONFLICT (...) DO UPDATE SET ... WHERE ... IS DISTINCT FROM EXCLUDED RETURNING xmax = 0`. Estimated speedup for appearances: 246s → 30–60s. Pairs naturally with the polars switch below — polars writes to a buffer or directly to COPY very efficiently. Validate: `just test-pipeline` covers the upsert helper; add a benchmark recipe (`just bench-pipeline`) that wraps `time just pipeline-force` for regression tracking.

- [ ] **Switch CSV parsing from pandas to polars.** Current `pd.read_csv(..., chunksize=CHUNK_SIZE)` + `chunk.itertuples()` does ~7,500 rows/sec end-to-end on appearances — most of the time is in per-row Python iteration after pandas parses. Polars (`pl.read_csv` + `iter_rows()` or vectorized expressions) is 3–10× faster on both parse and iteration. Better still: replace the row-loop with vectorized polars joins to resolve `transfermarkt_id` → internal `id` against player/club lookup DataFrames, then materialize the resolved rows in one shot. Touches all three ingest sites in `api/pipeline/ingest.py` + `api/pipeline/ingest_appearances.py`. New dependency (polars), small blast radius, no native-deps headaches. Validate: parity test against the existing fixture-driven SQLite suite (`just test`), then live `just pipeline-force` timing.
