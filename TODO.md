...
- [ ] **Make the tenure-success floor age-aware.** Today the floor in `_apply_tenure_success_floor` (`api/ml/scoring.py`) blanket-applies whenever `tenure_days >= 1460 AND minutes_pct >= 55%` — independent of player age. This creates a false-positive risk: a 22-year-old who plays 4 years at 60% minutes and is sold below cost should NOT have their financial outcome floored away (it likely signals regression / attitude issues / failure to develop). An older player (28+) sold below cost after the same stint IS age-appropriate depreciation and the floor is correct. Sketch: scale `floor_score` (or the override of `financial_return` specifically) by `age_at_transfer` — full effect at 28+, taper to zero by ~22. Possible shape: `effective_floor = floor_score * sigmoid((age_at_transfer - 25) / 2)`. Validate by re-running the curated list (none of the current 10 are young at transfer, so they should be unaffected) AND by checking the top-20 most-lifted transfers from the spot-check query in `GRADING_NOTES.md` for any young transfers that lose lift unfairly.
